...
**Constitution**: Aligned to v1.5.0 — includes refactoring tasks for compliance violations and new tasks for FR-044/045/046. v1.5.0 adds mandatory Google-style docstrings (Python) and JSDoc (TypeScript) for all functions/methods/classes (Principle III); CLI command handlers are exempt from Args/Returns documentation.

**Tests**: Included — Constitution v1.5.0 Principle VI mandates unit tests per module, integration tests per router, metamorphic tests per agent, deepeval pipelines per agent, and mutation testing (mutmut/Stryker). Test tasks are marked **[T]** in addition to any [P] marker.
...
- [x] T141 [P] [T] Write unit tests in `backend/tests/unit/test_auth.py` covering `get_current_user`, `hash_password`, `verify_password`, `require_study_member` (TREF6); write integration tests in `backend/tests/integration/test_auth_router.py` and `test_groups_router.py` covering all `/auth/*` and `/groups/*` endpoints including 401/403 error paths
- [x] T143 [P] [T] Write unit tests for `db/src/db/models/users.py` and `db/src/db/models/study.py` extensions in `db/tests/test_models_users.py` and `db/tests/test_models_study.py` — verify FK constraints, Enum validation, and timestamp defaults
...
- [x] T032b [US1] Extend `phase_gate.py` to track staleness timestamps: add `pico_saved_at`, `search_run_at`, and `extraction_started_at` fields to the `Study` model (or as a separate `StudyPhaseState` service object); add `compute_staleness_flags(study)` helper that returns a dict of `{phase: bool}` indicating whether a downstream phase has stale data (i.e., an upstream edit post-dates the downstream execution timestamp); update `PUT /pico`, `POST /searches/run`, and `POST /extractions/batch-run` to refresh the relevant timestamp; update `GET /studies/{study_id}` to include `stale_phases` in the response (FR-008a invalidation rules)
...
- [x] T146 [P] [T] Write unit tests for `SearchStringBuilderAgent` in `agents/tests/unit/test_search_builder.py` (mock LLMClient; verify boolean string output non-empty, terms_used present); write integration tests in `backend/tests/integration/test_criteria_router.py` and `test_search_strings_router.py` covering criteria CRUD, generate endpoint, test endpoint 202 response, iteration approval PATCH
- [x] T147 [P] [T] Write Vitest + Testing Library tests for `CriteriaForm.tsx`, `SearchStringEditor.tsx`, `TestRetest.tsx` in `frontend/src/components/phase2/__tests__/` — verify add/remove criterion interactions, generate button call, iteration recall display and approval button
...
- [x] T120 [P] Add `updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())` to `SearchString` in `db/src/db/models/search.py`; add `created_at: Mapped[datetime]` to `PICOComponent` in `db/src/db/models/pico.py` (NFR-001)
- [x] T121 [P] Add `updated_at` to `InclusionCriterion` and `ExclusionCriterion` in `db/src/db/models/criteria.py`; add `created_at` and `updated_at` columns to `SearchExecution` in `db/src/db/models/search_exec.py`; add `updated_at` to `BackgroundJob` in `db/src/db/models/jobs.py` and `SearchMetrics` in `db/src/db/models/search_exec.py` (NFR-001)
...
- [x] T123 Create Alembic migration `db/alembic/versions/0010_audit_trail_and_updated_at.py` covering: AuditRecord table creation + all missing `updated_at`/`created_at` column additions from T120–T122 — use `revision = "0010"`, `down_revision = "0009"` (chaining after T093's results migration)
- [x] T124 [P] Export `AuditRecord` from `db/src/db/__init__.py` alongside existing model exports
...
- [x] T063 [US3] Implement `backend/src/backend/services/dedup.py`: two-stage dedup — (1) exact DOI match against existing CandidatePapers for study, (2) rapidfuzz title similarity ≥ 0.90 + author overlap → probable duplicate flagged for review; returns `DedupResult(is_duplicate, definite, candidate_id_if_dup)`
- [x] T064 [US3] Extend `ScreenerAgent` in `agents/src/agents/services/screener.py` to accept structured InclusionCriterion/ExclusionCriterion lists and return `ScreeningResult(decision: accepted/rejected/duplicate, reasons: [{criterion_id, type, text}])` Pydantic model
...
- **Constitution compliance (v1.5.0)**:
  - TREF tasks (TREF1–TREF6) are BLOCKING — resolve before continuing any unchecked tasks (Principles II, III, VII)
  - All new Python functions/methods/classes MUST have Google-style docstrings with `Args:`, `Returns:`, and `Raises:` sections where applicable; CLI command handlers MUST include a brief description only — no Args/Returns (Principle III, v1.5.0)
  ...
  - SC-012 (audit trail renders ≤3s for ≤500 events): automated performance regression in T180; manual validation also required during T118 (record observed time in PR description)
  - Export service (T098/T135) MUST redact all Settings-derived values from exported payloads (Principle VIII / FR-046 / NFR-003)
  - NFR-003 (secrets outside application artefacts): satisfied by TREF5 (no hardcoded secret defaults), T135–T137 (export redaction), and `.env`-based configuration per docker-compose; no secrets may appear in any exported JSON/CSV/archive
  - NFR-004 (independently health-checked Docker services): satisfied by T131–T134 (admin health dashboard) and existing Docker Compose healthcheck blocks on all services; verify each service has `healthcheck:` in `docker-compose.yml` before marking T134 done
