...
- **Protocol Executor** (`backend/src/backend/services/protocol_executor.py`): `ProtocolAssignmentService` — `assign_protocol`, `reset_to_default`, `activate_eligible_tasks`; quality gate evaluation dispatch (`ProtocolGateEvaluator`); metric readers for `kappa_coefficient`, `accepted_paper_count`, `test_set_recall`, `coverage_recall` (corrected 2026-08-08 — this list previously named `paper_count`, `paper_screened_ratio` and `qa_completeness`, none of which exist; the absence of `qa_completeness` is load-bearing, see **G51**); `CompleteTaskService.complete_task` and `ApproveTaskService.approve_task` for runtime execution
- **YAML Import/Export** (`backend/src/backend/services/protocol_yaml.py`): `ProtocolYamlService.export()` serialises a `ResearchProtocol` to validated YAML (`ProtocolExportSchema`, schema version `"1.0"`); `import_yaml()` parses and persists as new protocol via `create_protocol()`
- **Protocol REST API** (`backend/src/backend/api/v1/protocols/`): `library.py` — `GET/POST /protocols`, `GET/PUT/DELETE /protocols/{id}`, `GET /protocols/{id}/export`, `POST /protocols/import`; `assignment.py` — `GET/PUT/DELETE /studies/{id}/protocol-assignment`, `GET /studies/{id}/execution-state`; `execution_state.py` — `POST /studies/{id}/execution-state/{task_id}/complete`, `POST /studies/{id}/execution-state/{task_id}/approve`
- **Frontend Protocol Components** (`frontend/src/components/protocols/`): `ProtocolGraph` (D3 force-directed read/edit SVG, drag-to-reposition), `ProtocolNodePanel` (read/edit MUI Drawer), `ProtocolTextEditor` (YAML `<textarea>` with parse errors), `ProtocolList` (MUI List with Copy/Assign/Export actions), `QualityGateEditor`, `EdgeConditionBuilder`, `ExecutionStateView` (task status table, Mark Complete / Approve buttons)
- **Frontend Protocol Pages** (`frontend/src/pages/protocols/`): `ProtocolLibraryPage` (list, filter by study type, copy, import, assign, export), `ProtocolEditorPage` (dual-pane graph + YAML editor, Save / conflict dialog); both registered at `/protocols` and `/protocols/:id` in `App.tsx`
- **Frontend Hooks** (`frontend/src/hooks/protocols/`): `useProtocol.ts` (list, detail, assignment queries; copy/create/update/delete/import/reset/assign mutations), `useExecutionState.ts` (5 s poll while ACTIVE, complete/approve mutations), `useProtocolEditor.ts` (`useReducer` graph ↔ YAML sync, 300 ms debounce), `useProtocolD3.ts` (D3 force simulation + drag)
