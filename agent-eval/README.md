...
The `agent_generator_eval` pipeline evaluates the quality of system messages generated by
`AgentGeneratorAgent`.

```bash
uv run agent-eval evaluate \
  --agent agent_generator \
  --suite agent-eval/test-suites/agent_generator.jsonl \
  --threshold 0.8 \
  --output /tmp/agent-generator-eval.json
```
...
│   │   └── improve.py
│   ├── pipelines/
│   │   └── agent_generator_eval.py  # DeepEval pipeline for AgentGeneratorAgent
│   └── judge/
