...
describe('ExportPanel', () => {
  ...
  describe('Negative display states', () => {
    it('"Export ready" NOT shown when job is running', async () => {
      vi.useFakeTimers({ shouldAdvanceTime: true });
      ...
      await waitFor(() => expect(mockApi.post).toHaveBeenCalledTimes(1));
      await vi.advanceTimersByTimeAsync(2500);
      await waitFor(() => screen.queryByText(/exporting/i));
      expect(screen.queryByText(/export ready/i)).toBeNull();
    });

    it('error message NOT shown initially before any export attempt', () => {
      const { container } = render(<ExportPanel studyId={STUDY_ID} />);
    ...
    it('radio buttons NOT disabled when job status is failed', async () => {
      vi.useFakeTimers({ shouldAdvanceTime: true });
    ...
    it('"Export ready" NOT shown when job is running (not completed)', async () => {
      vi.useFakeTimers({ shouldAdvanceTime: true });
      ...
      await waitFor(() => expect(mockApi.post).toHaveBeenCalledTimes(1));
      await vi.advanceTimersByTimeAsync(2500);
      await waitFor(() => screen.queryByText('50%'));
      expect(screen.queryByText(/export ready/i)).toBeNull();
    });

    it('size_bytes span NOT shown when progress_detail is null in completed job', async () => {
      vi.useFakeTimers({ shouldAdvanceTime: true });
      ...
      await vi.advanceTimersByTimeAsync(2500);
      await waitFor(() => screen.queryByRole('button', { name: /download/i }));
      // No size indicator when progress_detail is null
      expect(screen.queryByText(/ B$/)).toBeNull();
      expect(screen.queryByText(/ KB$/)).toBeNull();
