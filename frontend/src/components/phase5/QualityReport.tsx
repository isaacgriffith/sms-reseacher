...
const RUBRICS: Array<{
  ...
  {
    key: 'score_search_evaluation',
    name: 'Search Evaluation',
    max: 3,
    rubricKey: 'search_evaluation',
  },
  {
    key: 'score_extraction_classification',
    name: 'Extraction & Classification',
    max: 3,
    rubricKey: 'extraction_classification',
  },
...
function RubricCard({
  ...
  return (
    <Paper
      variant="outlined"
      sx={{ border: '1px solid #e2e8f0', borderRadius: '0.375rem', padding: '0.75rem 1rem' }}
    >
      ...
      {detail?.justification && (
        <Typography
          sx={{ marginTop: '0.5rem', fontSize: '0.8125rem', color: '#6b7280', lineHeight: 1.5 }}
        >
          {detail.justification}
        </Typography>
      )}
  ...
  if (!recommendations.length) {
    return (
      <Typography sx={{ color: '#6b7280', fontSize: '0.875rem' }}>
        No recommendations — all rubrics scored well.
      </Typography>
    );
  }
        ...
        return (
          <li key={i}>
            ...
            >
              ...
              <Typography sx={{ margin: 0, fontSize: '0.875rem', color: '#374151' }}>
                {rec.action}
              </Typography>
              <Button
                variant="outlined"
...
export default function QualityReport({ studyId }: QualityReportProps) {
  ...
  const loadReports = async () => {
    ...
    setError(null);
    try {
      const data = await api.get<QualityReportSummary[]>(
        `/api/v1/studies/${studyId}/quality-reports`,
      );
      setSummaries(data);
      if (data.length > 0) {
  ...
  const loadDetail = async (reportId: number) => {
    try {
      const detail = await api.get<QualityReportData>(
        `/api/v1/studies/${studyId}/quality-reports/${reportId}`,
      );
  ...
  return (
    ...
    >
      ...
      {/* Error */}
      {error && (
        <Typography sx={{ color: '#dc2626', fontSize: '0.875rem', marginBottom: '0.75rem' }}>
          {error}
        </Typography>
      )}
      ...
      {summaries.length > 1 && (
        <Box sx={{ marginBottom: '1rem' }}>
          <Typography
            component="label"
            sx={{ fontSize: '0.8125rem', color: '#6b7280', marginRight: '0.5rem' }}
          >
            Report version:
          </Typography>
          <select
      ...
      {!loading && !selectedReport && summaries.length === 0 && (
        <Box sx={{ textAlign: 'center', padding: '2rem 0', color: '#9ca3af' }}>
          ...
          <Typography sx={{ fontSize: '0.875rem' }}>
            Click "Run Evaluation" to score this study.
          </Typography>
        </Box>
      )}
    </Paper>
