...
describe('TestRetest', () => {
  ...
  describe('Run test button style states', () => {
    it('Run Test Search button has pointer cursor when string is available', async () => {
      mockApi.get.mockResolvedValueOnce(MOCK_SEARCH_STRINGS);
      renderWithQuery(<TestRetest studyId={1} />);
      await waitFor(() => screen.getByRole('button', { name: /run test search/i }));
      const btn = screen.getByRole('button', { name: /run test search/i }) as HTMLButtonElement;
      expect(btn.style.cursor).toBe('pointer');
    });

    it('Run Test Search button has opacity 1 when string is available', async () => {
      mockApi.get.mockResolvedValueOnce(MOCK_SEARCH_STRINGS);
      renderWithQuery(<TestRetest studyId={1} />);
      await waitFor(() => screen.getByRole('button', { name: /run test search/i }));
      const btn = screen.getByRole('button', { name: /run test search/i }) as HTMLButtonElement;
  ...
  describe('Negative error/success states', () => {
    it('test error NOT shown initially before any run attempt', async () => {
      mockApi.get.mockResolvedValueOnce(MOCK_SEARCH_STRINGS);
      const { container } = renderWithQuery(<TestRetest studyId={1} />);
      await waitFor(() => screen.getByRole('button', { name: /run test search/i }));
      expect(screen.queryByText(/test search failed/i)).toBeNull();
    ...
    it('"[AI]" NOT shown for non-AI strings in selector', async () => {
    ...
    it('recall value shown as exact percentage with one decimal', async () => {
      // test_set_recall: 0.75 → '75.0%'
      mockApi.get.mockResolvedValueOnce(MOCK_SEARCH_STRINGS);
      renderWithQuery(<TestRetest studyId={1} />);
      await waitFor(() => {
        expect(screen.getByText('75.0%')).toBeTruthy();
